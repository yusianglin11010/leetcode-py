...
- two pointer
    - use two pointer to go through two linked lists
    - while reach the end of a linked list, move forward to another linked list
- calculate length of linked list
    - for two linked list, if they cross, the two pointer would meet each other while they start moving in the same index
    - all we need is to make `headA` and `headB` in the same index, that is of `len(A)` is longer than `len(B)`, then we move headA `len(A) - len(B)` forward to make `headA` and `headB` the same index